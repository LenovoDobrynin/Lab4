		...
		public static void Main(string[] args)
		{
			...
			for (int i = 0; i < trackList.Length; i++)
			{
				...
				foreach (Match m in min)
				{
					...
					Console.Write(digitMin + " мин, ");
					sum += digitMin * 60;
				}
				foreach (Match s in sec)
				{
					digitSec = int.Parse(s.Value);
					Console.Write(digitSec + " сек\n");
					sum += digitSec;
				}
				Console.WriteLine();
			...
			Console.WriteLine();
			Console.WriteLine("Общее время плейлиста: " + sum + " сек");
			Console.WriteLine("Самая короткая песня: " + shortestSong);
			Console.WriteLine("Самая длинная песня: " + longestSong);
			Console.WriteLine("Песни  минимальной разницей во времени звучания: {0} and {1}", trackList[firstTrackIndex], trackList[secondTrackIndex]);
